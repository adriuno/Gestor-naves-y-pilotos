  ...
  <div v-if="!autenticando" >
      ...
      <div class="flex justify-center mt-6">
        ...
        <div class="relative w-full max-w-md">
          <input v-model="busqueda" type="text" placeholder="Buscar nave/piloto..."
            ...
            aria-label="Buscar nave o piloto" @focus="mostrarSugerencias = true" @blur="ocultarSugerenciasConRetardo"
            @input="actualizarSugerencias" />

          <!-- Icono de lupa -->
          <div class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" aria-hidden="true">
      ...
      <div class="min-h-[300px] mt-8">
        <div v-if="paginatedStarships.length"
          class="grid grid-cols-1 sm:grid-cols-2 gap-x-16 gap-y-14 p-6 mx-auto max-w-screen-2xl mt-8">
          <div v-for="starship in paginatedStarships" :key="starship.id"
            ...
            <div class="w-full md:w-[26rem] h-60 md:h-auto overflow-hidden">
              <img :src="starship.image2_url" :alt="starship.name"
            ...
            <div class="flex flex-col justify-between p-4 text-gray-300 flex-1">
              <h5
                class="font-serif bg-gradient-to-br from-gray-400 via-gray-600 mb-1 to-gray-800 text-yellow-300 px-3 py-1 rounded-full shadow-inner shadow-yellow-600/20 border border-gray-400 text-2xl font-semibold">
                {{ starship.name }}
              </h5>
              <p class="mt-8 text-center md:text-left font-serif">
                <strong
                  class="bg-gradient-to-br from-gray-400 via-gray-600 mb-1 to-gray-800 text-yellow-300 px-3 py-1 rounded-full shadow-inner shadow-yellow-600/20 border border-gray-400 text-sm font-semibold">Modelo:</strong>
                {{ starship.model }}
              </p>
              <p class="text-center md:text-left mt-4 font-serif ">
                <strong
                  class="bg-gradient-to-br from-gray-400 via-gray-600 mb-1 to-gray-800 text-yellow-300 px-3 py-1 rounded-full shadow-inner shadow-yellow-600/20 border border-gray-400 text-sm font-semibold">Pilotos:</strong>
                {{ starship.pilots.length }}
                <button aria-label="Añadir piloto"
              ...
              <div class="mt-4 flex flex-col items-center gap-4 w-full">
                ...
                <div v-if="showPilotForm === starship.id" class="space-y-2 w-full">
                  <select v-model="selectedPilotId"
                  ...
                  <div class="flex gap-2 justify-center">
                    ...
                    <button aria-label="Confirmar selección de piloto"
                    ...
                    <button aria-label="Cancelar selección de piloto"
                      class="py-2 px-4 bg-red-600 text-white rounded-xl hover:bg-red-500 transition" @click="cancelar">
              ...
              <div class="mt-auto flex justify-center mb-6">
                <button
                  class="px-4 py-2 text-sm font-medium text-white bg-yellow-600 rounded-lg transition hover:drop-shadow-[0_0_15px_#38bdf8]"
      ...
      <transition name="modal-fade">
        <div v-if="showModal" class="fixed inset-0 bg-opacity-50 flex backdrop-blur-sm justify-center items-center z-50"
          role="dialog" aria-modal="true" aria-labelledby="titulo-modal" aria-describedby="contenido-modal"
          ...
          <div ref="modalRef" tabindex="-1"
            ...
            <div class="flex justify-center mb-4">
              <img :src="modalStarship.image2_url" :alt="`Imagen de la nave ${modalStarship.name}`"
            ...
            <div class="mx-auto text-left w-fit space-y-2" role="group">
              <p class="text-sm sm:text-base">
                <strong class="mr-2 bg-yellow-500 text-black px-3 py-1 rounded-full shadow font-semibold">
                  Modelo:
                </strong>
              ...
              <p class="text-sm sm:text-base">
                <strong class="bg-yellow-500 text-black px-3 py-1 rounded-full shadow font-semibold mr-2">
                  Coste:
                </strong>
            ...
            <!-- Pilotos -->
            <div class="mt-8" role="region" aria-labelledby="heading-pilotos" tabindex="0">
              <!-- Título con ID para aria-labelledby -->
              <h4 id="heading-pilotos" class="text-xl font-bold mb-2 bg-yellow-500 text-black py-1 px-2 rounded-xl">
                Pilotos:
              </h4>

              <!-- Si no hay pilotos -->
              <div v-if="modalStarship.pilots.length === 0"
                class="text-yellow-500 text-center hover:bg-gray-700/50 hover:rounded-4xl rounded-lg px-4 py-2"
              ...
              <div v-else-if="modalStarship.pilots.length === 1"
                class="max-h-64 overflow-y-auto pr-2 flex justify-center" role="list"
                ...
                <div v-for="pilot in modalStarship.pilots" :key="pilot.id" role="listitem"
                  ...
                  <div class="flex items-center gap-2">
                    <img :src="pilot.image_url" class="w-12 h-12 rounded-full border border-yellow-500 object-cover" />
                    <span class="text-md text-white">{{ pilot.name }}</span>
                  </div>
                  <button :aria-label="`Eliminar al piloto ${pilot.name}`"
              ...
              <div v-else class="max-h-64 overflow-y-auto w-2xl ml-8 pr-2 grid grid-cols-2 gap-x-27 gap-1" role="list"
                ...
                <div v-for="pilot in modalStarship.pilots" :key="pilot.id" role="listitem"
                  ...
                  <button :aria-label="`Botón para eliminar al piloto ${pilot.name}`"
                    class="text-white hover:scale-120 hover:bg-red-600 p-3 rounded-lg transition"
                    @click="deletePilot(modalStarship.id, pilot.id)">
                    <i class="fas fa-trash" aria-hidden="true" />
                  </button>
              ...
              <div class="mt-2">
                <button
                  ...
                  aria-label="Cerrar ventana modal" @click="closeModal">
                  Cerrar
                </button>
              </div>
            </div>
...
const fetchAvailablePilots = async (starshipId) => {
  try {
    ...
    // Ordenamos los pilotos por nombre
    availablePilots.value = res.data.sort((a, b) => a.name.localeCompare(b.name));
